...
import { deleteIncome, getIncomes } from "@firebaseAPI"
...
export default function StudentInfoIncome({ navigation, route }) {
	...
	const to = Math.min((page + 1) * numberOfItemsPerPage, incomes.length)
	const dataPerPage = incomes.slice(
	...
	useFocusEffect(
		useCallback(() => {
			const getIncomesFB = async () => {
				const incomesDB = await getIncomes(data.user.uid, studentKey)
				const incomesInfo = incomesDB
					? Object.keys(incomesDB).map((key) => {
						const newObj = Object.create({ key, moneyInfo: {} })
						newObj.key = key
						newObj.moneyInfo = incomesDB[key]
						return newObj
					})
					: []
				setIncomes(incomesInfo)
			}
			getIncomesFB()
		}, [data.user.uid, studentKey, incomes])
	)
	...
	const handleDeleteRow = (incomeKey) => {
		return Alert.alert("Deletar fileira?", "As informações serão apagadas", [
			...
			},
			{
				...
				style: "destructive",
				onPress: async () => {
					deleteIncome(data.user.uid, studentKey, incomeKey)
				},
			},
		])
	}
		...
		<View className="flex items-center h-screen">
            ...
            Total:
				...
				<DataTable.Pagination
					page={page}
					numberOfPages={Math.ceil(incomes.length / numberOfItemsPerPage)}
					onPageChange={(page) => setPage(page)}
					label={`${from + 1}-${to} de ${incomes.length}`}
			...
			<Button
				mode="text"
				onPress={() =>
					handleNavigate(navigation, "CreateStudentInfoIncome", { studentKey })
				}
			>
        CRIAR
			</Button>
